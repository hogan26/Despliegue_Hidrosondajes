                ...
                if opportunity_id:
                    for sale_order in opportunity_id.order_ids:
                        if sale_order.state=="sale":
                            ...
                            #USO DEL CAMPO SERVICIOS REQUERIDOS PARA IDENTIFICAR CUANTOS SERVICIOS CONTIENE Y EL ORDEN DE ESTOS.
                            servicios_requeridos = sale_order.x_servicios_requeridos
                            filtro_servicios = []
                            if servicios_requeridos == 's1s2':
                                filtro_servicios = ['s1','s2']
                            elif servicios_requeridos == 's1s2s3':
                                filtro_servicios = ['s1','s2','s3']
                            elif servicios_requeridos == 's2s3':
                                filtro_servicios = ['s2','s3']
                            else:
                                filtro_servicios.append(self.pending_settlements_select)
                                ...
                                if section_line['type'] in settlement_services:                                    
                                    for settlement_line in sale_order.order_line: 
                                        if line_index<section_line['start']:
                                            ...
                                            if line_index == section_line['end']:
                                                break
                                            else:
                                                line_index += 1
...
#                                                             _logger.info('medida_pulgadas = {}, suma_total = {}'.format(validation_data['medida_pulgadas'],validation_data['suma_total']))
                    ...
                    raise ValidationError("No se pudo encontrar el requerimiento")
